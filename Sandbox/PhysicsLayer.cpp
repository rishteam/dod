...
{
    ImGui::LoadIniSettingsFromDisk("assets/layout/physics_layer.ini");
    m_world = MakeRef<rl::PhysicsWorld>(Vec2(0.0f, -9.8f), 800.0f, 600.0f);
}

void PhysicsLayer::onDetach()
{
    ImGui::SaveIniSettingsToDisk("assets/layout/physics_layer.ini");
}

void PhysicsLayer::onUpdate(rl::Time dt)
{
    ...
    Renderer2D::BeginScene(m_cameraController.getCamera(), true);
    {
        ...
        //draw box
        for (const auto& obj : m_world->bodies) {
            Renderer2D::DrawRotatedQuad({obj->position.x, obj->position.y}, {obj->wh.x, obj->wh.y}, {1, 0, 0, 0.5},
                                        glm::degrees(obj->angle));
        }
        for (const auto& jit : m_world->joints) {
...
void PhysicsLayer::onEvent(rl::Event &event)
{
