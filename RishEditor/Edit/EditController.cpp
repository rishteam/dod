...
//            auto boundSize = std::min(bound.getScale().x,bound.getScale().y)*0.1;
            ...
            switch( m_gizmoMode ) {
                ...
                case Gizmo::ScaleMode:{
                    ...
                    Renderer2D::DrawLine(entPos,pointX,glm::vec4(0.f,0.f,1.f,1.f));
                    Renderer2D::DrawQuad(pointX,boundSize,boundColor);
                    Renderer2D::DrawLine(entPos,pointY,glm::vec4(1.f,0.f,0.f,1.f));
                    Renderer2D::DrawQuad(pointY,boundSize,boundColor);
                    break;
                }
                ...
                for (auto ent : m_entitySet) {
                    ...
                    if (!m_isNowMovingEntity[ent]) {
                        ...
                        m_oriEntityNegative[ent] = glm::vec3(entSize.x > 0 ? 1.f : -1.f, entSize.y > 0 ? 1.f : -1.f,
                                                             1.f);
                        m_isNowMovingEntity[ent] = true;
                        if (Math::AABB2DPoint(boundPos + boundSize * glm::vec3(.5f, .5f, 1.f), glm::vec3(.1f),
                                              mposInWorld)) { // right up
                            m_zoomEntityWeight[ent] = glm::vec3(1.f, 1.f, 0.f);
                            m_moveEntityWeight[ent] = glm::vec3(.5f, .5f, 0.f);
                        } else if (Math::AABB2DPoint(boundPos + boundSize * glm::vec3(.5f, 0.f, 1.f), glm::vec3(.1f),
                                                     mposInWorld)) { // right
                            m_zoomEntityWeight[ent] = glm::vec3(1.f, 0.f, 0.f);
                            m_moveEntityWeight[ent] = glm::vec3(.5f, 0.f, 0.f);
                        } else if (Math::AABB2DPoint(boundPos + boundSize * glm::vec3(.5f, -.5f, 1.f), glm::vec3(.1f),
                                                     mposInWorld)) { // right down
                            m_zoomEntityWeight[ent] = glm::vec3(1.f, -1.f, 0.f);
                            m_moveEntityWeight[ent] = glm::vec3(.5f, .5f, 0.f);
                        } else if (Math::AABB2DPoint(boundPos + boundSize * glm::vec3(0.f, -.5f, 1.f), glm::vec3(.1f),
                                                     mposInWorld)) { // down
        ...
        case Gizmo::ScaleMode :{
            ...
            if (m_sceneWindowFocused &&
                ...
                for (auto ent : m_entitySet) {
                    ...
                    const glm::vec3 boundSize(.1f);
                    // The entity is not moving but selected
                    if (!m_isNowMovingEntity[ent]) {
                        // Is mouse inside
                        if (Math::AABB2DPoint(pointX, boundSize, mposInWorld)) {
                            m_oriEntityPosition[ent] = entPos;
                            m_oriEntitySize[ent] = entSize;
                            m_oriMousePosition[ent] = glm::vec3(mposInWorld, 0.f);
                        ...
                        if(Math::AABB2DPoint(pointY, boundSize, mposInWorld)){
                            m_oriEntityPosition[ent] = entPos;
                            m_oriEntitySize[ent] = entSize;
                            m_oriMousePosition[ent] = glm::vec3(mposInWorld, 0.f);
